...
const getBaseUrl = () => {
  ...
  return raw.replace(/\/$/, ""); // last slash remove
};
...
const sendOrderPlacedEmail = async ({ to, userName, orderId, total }) => {
  ...
  const html = `
    <div style="font-family: Arial; padding: 16px; max-width:600px;">
      <h2>Order Placed ✅</h2>
...
/**
 * ✅ Delivered mail with Invoice download button/link
 * IMPORTANT: invoice route is protected (needs login cookie), so user must be logged in.
 */
const sendOrderDeliveredEmail = async ({ to, userName, orderId, productName }) => {
  const subject = `Order Delivered ✅ (#${orderId.slice(-6).toUpperCase()})`;

  const invoiceUrl = `${getBaseUrl()}/api/orders/${orderId}/invoice`;

  const html = `
    <div style="font-family: Arial; padding: 16px; max-width:600px;">
      <h2>Order Delivered ✅</h2>
      <p>Hi ${userName || "Customer"}, your order has been delivered successfully.</p>
...
module.exports = {
  ...
  sendOrderDeliveredEmail, // ✅ exported
